	...
	class Taxonomy_Core {
		...
		public function __construct( $taxonomy, $arg_overrides = array(), $object_types = array( 'post' ) ) {
			...
			if ( ! is_array( $taxonomy ) ) {
				wp_die( __( 'It is required to pass a single, plural and slug string to Taxonomy_Core', '_s' ) );
			}
		...
		public function get_args() {
			...
			$labels = array(
				...
				'update_item'                => sprintf( __( 'Update %s', '_s' ), $this->singular ),
				'add_new_item'               => sprintf( __( 'Add New %s', '_s' ), $this->singular ),
				'new_item_name'              => sprintf( __( 'New %s Name', '_s' ), $this->singular ),
				'not_found'                  => sprintf( __( 'No %s found.', '_s' ), $this->plural ),
				'no_terms'                   => sprintf( __( 'No %s', '_s' ), $this->plural ),

				// Hierarchical stuff
				'parent_item'       => $hierarchical ? sprintf( __( 'Parent %s', '_s' ), $this->singular ) : null,
				'parent_item_colon' => $hierarchical ? sprintf( __( 'Parent %s:', '_s' ), $this->singular ) : null,

				// Non-hierarchical stuff
				'popular_items'              => $hierarchical ? null : sprintf( __( 'Popular %s', '_s' ), $this->plural ),
				'separate_items_with_commas' => $hierarchical ? null : sprintf( __( 'Separate %s with commas', '_s' ), $this->plural ),
				'add_or_remove_items'        => $hierarchical ? null : sprintf( __( 'Add or remove %s', '_s' ), $this->plural ),
				'choose_from_most_used'      => $hierarchical ? null : sprintf( __( 'Choose from the most used %s', '_s' ), $this->plural ),
			);
