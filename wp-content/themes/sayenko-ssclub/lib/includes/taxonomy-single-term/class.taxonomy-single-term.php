...
class Taxonomy_Single_Term {
	...
	public function select_open() {
		...
		<select style="display:block;width:100%;margin-top:12px;" name="<?php echo $this->namefield; ?>" id="<?php echo $this->slug; ?>checklist" class="form-no-clear">
			<?php if ( ! $this->force_selection ) : ?>
				<option value="0"><?php echo esc_html( apply_filters( 'taxonomy_single_term_select_none', __( 'None', '_s' ) ) ); ?></option>
			<?php endif;
	}
	...
	public function radio_close() {
		...
		<p style="margin-bottom:0;float:left;width:50%;">
			<a class="button" id="taxonomy-<?php echo $this->slug; ?>-trigger-clear" href="#"><?php _e( 'Clear', '_s' ); ?></a>
		</p>
	...
	public function terms_adder_button() {
		...
		<p style="margin-bottom:0;float:right;width:50%;text-align:right;">
			<a class="button-secondary" id="taxonomy-<?php echo $this->slug; ?>-new" href="#"<?php if ( 'radio' == $this->input_element ) : ?> style="display:inline-block;margin-top:0.4em;"<?php endif; ?>><?php _e( 'Add New', '_s' ); ?></a>
		</p>
		<script type="text/javascript">
			jQuery(document).ready(function($){
					...
					if(termName != null) {
						...
						$.post( ajaxurl, data, function(response) {
							...
							if( response.success ){
								...
								window.alert( '<?php printf( __( 'There was a problem adding a new %s', '_s' ), esc_attr( $this->taxonomy()->labels->singular_name ) ); ?>: ' + "\n" + response.data );
							}
						});
					}
				});
	...
	function ajax_add_term() {
		...
		if( !$this->allow_new_terms ) {
			wp_send_json_error( __( "New $friendly_taxonomy terms are not allowed", '_s' ) );
		}

		if( !taxonomy_exists( $taxonomy ) ) {
			wp_send_json_error( __( "Taxonomy $friendly_taxonomy does not exist. Cannot add term", '_s' ) );
		}

		if( !wp_verify_nonce( $nonce, 'taxonomy_' . $taxonomy, '_add_term' ) ) {
			wp_send_json_error( __( "Cheatin' Huh? Could not verify security token", '_s' ) );
		}

		if( term_exists( $term_name, $taxonomy ) ) {
			wp_send_json_error( __( "The term '$term_name' already exists in $friendly_taxonomy", '_s' ) );
		}
